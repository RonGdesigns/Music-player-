...
The date a song was added, which Recently added and sorting by date go by,
never changes, because the file is rewritten in place rather than replaced. The
file's modified date does change, on purpose, so a file manager shows when a
save (or an undo) last touched it.
